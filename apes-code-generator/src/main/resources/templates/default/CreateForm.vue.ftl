...
export default defineComponent({
    ...
    setup(props): CreateFormSetupData {
        ...
        // 表单值
        const modelRef = reactive<Omit<${tableInfo.className}DataType, '<#list tableInfo.colInfoList as field><#if field.primaryKey>${field.javaColumnName}</#if></#list>'>>({
        <#-- ----------  BEGIN 字段循环遍历  ---------->
        <#list tableInfo.colInfoList as field>
            <#if field.frontShow==false><#continue /></#if>
            <#if field.primaryKey==true> <#continue /></#if>
            ///${field.comment}
            <#if field.tsColumnType=='string'>
        ...
        const onFinish = async () => {
            try {
                const fieldsValue = await validate<Omit<${tableInfo.className}DataType, '<#list tableInfo.colInfoList as field><#if field.primaryKey>${field.javaColumnName}</#if></#list>'>>();
